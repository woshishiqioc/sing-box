...
func (p *ProxyProvider) StartGetOutbounds() ([]option.Outbound, error) {
	p.logger.Info("getting outbounds from provider")
	if p.cacheFile != "" {
		if rw.FileExists(p.cacheFile) {
			...
			if !cache.IsNil() {
				p.cache = new(Cache)
				*p.cache = cache
				p.logger.Info("load cache file done")
			} else {
	...
	if p.cache == nil || (p.cache != nil && p.updateInterval > 0 && p.cache.LastUpdate.Add(p.updateInterval).Before(time.Now())) {
		p.logger.Info("updating outbound provider")
		cache, err := p.wrapUpdate(p.ctx, true)
		if err != nil {
			if p.cache == nil {
				return nil, E.Cause(err, "load outbound provider failed")
			} else {
				p.logger.Warn("update outbound provider failed: ", err)
			}
		}
		p.cache = cache
		if p.cacheFile != "" {
		...
		p.logger.Info("update outbound provider done")
	}
	defer func() {
...
func (p *ProxyProvider) getFullOutboundOptions(ctx context.Context) ([]option.Outbound, error) {
	...
	p.logger.Info("get outbounds from provider done")
...
func (p *ProxyProvider) update(ctx context.Context, isFirst bool) {
	...
	if err != nil {
		p.logger.Error("update outbound provider failed: ", err)
		return
	}
	p.cacheLock.Lock()
	p.cache = cache
