...
func (c *ClashShadowsocks) GenerateOptions() (*option.Outbound, error) {
	...
	case "v2ray-plugin":
		v2rayOptions := make(map[string][]string)
		modeAny, ok := c.PluginOpts["mode"]
		if ok {
			mode, ok := modeAny.(string)
			if ok {
				v2rayOptions["obfs"] = []string{mode}
			}
		}
		hostAny, ok := c.PluginOpts["host"]
		if ok {
			host, ok := hostAny.(string)
			if ok {
				v2rayOptions["obfs-host"] = []string{host}
			}
		}
		pathAny, ok := c.PluginOpts["path"]
		if ok {
			path, ok := pathAny.(string)
			if ok {
				v2rayOptions["path"] = []string{path}
			}
		}
		tlsAny, ok := c.PluginOpts["tls"]
		if ok {
			tls, ok := tlsAny.(bool)
			if ok {
				v2rayOptions["tls"] = []string{strconv.FormatBool(tls)}
			}
		}
		muxAny, ok := c.PluginOpts["mux"]
		if ok {
			mux, ok := muxAny.(bool)
			if ok {
				v2rayOptions["mux"] = []string{strconv.FormatBool(mux)}
			}
		}
		pluginOptsStr := encodeSmethodArgs(v2rayOptions)
		outboundOptions.ShadowsocksOptions.Plugin = "v2ray-plugin"
		outboundOptions.ShadowsocksOptions.PluginOptions = pluginOptsStr
	default:
